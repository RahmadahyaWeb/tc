...
$url =  \yii\helpers\Url::to(['trplafon/update', 'id' => $model->id]);
$urlEdit = \yii\helpers\Url::to(['trplafon/edit', 'id' => $model->id]);
$urlRedirect = \yii\helpers\Url::to(['trplafon/view', 'id' => $model->id]);
$biaya_awal = $model->biaya;
$tgl_invoice = $model->tgl_invoice;
            ...
            <div class="col-md-4">
                <div class="form-group">
                    ...
                    <select id="trplafon-status_benefit" class="form-control">
                        ...
                        <option value="Ya" <?= ($model->nonbenefit !== "*") ? "selected" : "" ?>>Ya</option>
                        <option value="Tidak" <?= ($model->nonbenefit === "*") ? "selected" : "" ?>>Tidak</option>
                    </select>
            ...
            <?php if ($cekOver) : ?>
                <div class="col-md-4">
                    ...
                    echo NumberControl::widget([
                        ...
                        'displayAttribute' => 'biaya_inputan',
                    ]);
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <label for="">Biaya Plafon</label>
                        <input type="text" class="form-control text-right" value="<?= 'Rp. ' . number_format($model->biaya, 0, ',', '.') ?>" disabled>
                    </div>
                </div>
            <?php else : ?>
                <div class="col-md-4">
            ...
            success: function(response) {
                ...
                if (namaPlafon == "KACAMATA") {
                    ...
                    if (!success) {
                        ...
                        if (confirmed) {
                            let urlEditClean = removePHPTags('<?= $urlEdit ?>');
                            let urlEdit = urlEditClean + '&sisaPlafon=' + responseData.data.TrPlafon.sisaPlafon;
